...
import { matchDatabase } from '../src/lib/supabase';
import type { Match as SupabaseMatch } from '../src/lib/supabase';
import fs from 'fs';
...
const loadLastScrapeTime = async (): Promise<number> => {
  try {
    const lastScrapeTime = await matchDatabase.getLastScrapeTime();
    if (lastScrapeTime) {
      return new Date(lastScrapeTime).getTime();
    }
    return 0;
  } catch (error) {
...
// Save last scrape time to database
const saveScrapeTime = async () => {
  try {
    await matchDatabase.updateLastScrapeTime();
  } catch (error) {
    console.error('Error saving scrape time:', error);
  }
};
...
export async function scrapeGAAFixturesAndResults(): Promise<SupabaseMatch[]> {
  ...
  try {
    ...
    // When processing matches, store them in Supabase
